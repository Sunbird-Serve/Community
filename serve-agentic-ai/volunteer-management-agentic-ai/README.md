...
Own the **volunteer** from the instant someone registers until they are serving a confirmed deliverable. Sub-goals = greet ➜ volunteer selection discussion ➜ decision (on the outcome) ➜ match ➜ assignment, while enforcing policy and emitting telemetry.

#### The Workflow - High Level

* **Registration event** → Orchestrator creates new run context.
* **Onboarding** → send welcome, gather slots, verify contacts, persist profile.
* **Screening** → schedule discussion, capture interview, auto-score & recommend.
* **Fulfillment** → run matching against open needs, confirm acceptance.
* **Assignment** → finalize in FulfillmentService, emit `need.assigned`.
...
* **Run Context (short-lived, per volunteerId):**\
  correlationId, timestamps, retry counters, OTP attempts, slot proposals, interview status.
* **Profile Context (long-lived, SoT):**\
  volunteer profile fields (subjects, grades, languages, availability, device, consent).
* **Interaction Memory (chat/session history):**\
  free-text replies, slot-filling turns, clarifications (stored in VectorDB for semantic recall).
* **Decision Memory (eligibility, screening outcome):**\
  interview scores, recommend/hold/reject status.
* **Telemetry (immutable events):**\
  `welcome_sent`, `otp_verified`, `screening.meeting.scheduled`, `assignment_success`.
...
| **Agentic AI**    | <p><strong>Volunteer-Coordinator Orchestrator</strong> ← orchestrates<br>OnboardingAgent<br>ScreeningAgent<br>FulfillmentAgent</p> | Agent policies, eligibility rules, state machine                                              |
...
* **Onboarding:** interpreting free-text replies → slot-filling (LLM).
* **Screening:** auto-scoring interviews → structured outcomes (LLM).
* **Fulfillment:** optional LLM-assisted matching if heuristic fails.
* **Everywhere:** Orchestrator provides guardrails (logic, retries, state mgmt).

### Failure/Edge Handling

* **OTP not verified:** fallback guidance; emit `onboarding.blocked`.
* **No slots:** queue volunteer; notify via comms; reschedule policy.
* **Duplicate events:** idempotency key (`volunteerId`, `meetingLink`).
* **Comms/Calendar outage:** retry with backoff; fallback to alternative channels.
* **LLM ambiguity:** clarification loop; fallback to structured form.
